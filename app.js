...
function setupEventListeners() {
    ...
    $('.month-tab').on('click', function() {
        // Save the current month's content before switching
        if (selectedObjective && selectedObjective.monthlyUpdates) {
            selectedObjective.monthlyUpdates[currentMonth] = $('#monthlyUpdates').val();
        }
...
function fetchObjectives() {
    ...
    // Filter by year AND where user is either assignee OR owner
    objectivesData = allObjectives.filter(obj =>
        obj.year === currentYear &&
        (obj.assigneeID === CURRENT_USER_ID || obj.ownerID === CURRENT_USER_ID)
    );
    ...
    console.log(`Displaying ${objectivesData.length} objectives for ${CURRENT_USER_ID}`);
    renderObjectives();
}
...
function openUpdateScreen(obj) {
    ...
    // Reset current month to system month every time form opens
    currentMonth = monthAbbreviations[new Date().getMonth()];

    // Populate form fields
    $('#updateTitle').text(obj.title);
    $('#updateDescription').val(obj.description || '');
    $('#updateAssignee').val(obj.assignee);
    $('#updateOwner').val(obj.owner);
    ...
    if (!canEdit) {
        ...
        $('#updateDescription').prop('readonly', true);
        ...
        $('#updateDescription').prop('readonly', false);
...
function openSubmitScreen() {
    ...
    // Auto-populate Assignee with current user if found in roles
    const currentUserRole = rolesData.find(role => role.id === CURRENT_USER_ID);
    if (currentUserRole) {
        $('#submitAssignee').val(CURRENT_USER_ID);
        console.log(`Auto-populated Assignee: ${currentUserRole.name} (${CURRENT_USER_ID})`);
    } else {
        console.log('Current user ID not found in roles, leaving Assignee as "Select Assignee"');
    }
...
function submitNewObjective() {
    ...
    // Validate that current user is either the assignee or owner
    if (CURRENT_USER_ID !== newObjective.assigneeID && CURRENT_USER_ID !== newObjective.ownerID) {
        alert('You can only submit objectives where you are either the Assignee or the Owner. You cannot submit on behalf of others.');
        return;
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.success) {
                // Add the new objective to sessionStorage if server returned the full objective
                if (data.objective) {
                    addObjectiveToSession(data.objective);
                    console.log('Added new objective to sessionStorage');
                }
                ...
                setTimeout(function() {
                    ...
                    closeSubmitScreen();
                    // Refresh objectives list from sessionStorage
                    fetchObjectives();
                }, 2000);
            } else {
...
function submitObjectiveUpdates() {
    ...
    // Save the current month's content before submitting
    if (!selectedObjective.monthlyUpdates) {
        selectedObjective.monthlyUpdates = {};
    }
    selectedObjective.monthlyUpdates[currentMonth] = $('#monthlyUpdates').val();
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.success) {
                // Update the selected objective with the new data
                selectedObjective.description = updatedData.description;
                selectedObjective.status = updatedData.status;
                selectedObjective.rag = updatedData.rag;
                selectedObjective.fromDate = updatedData.startDate;
                selectedObjective.toDate = updatedData.targetDate;
                selectedObjective.milestones = updatedData.milestones;
                selectedObjective.metrics = updatedData.metrics;
                selectedObjective.monthlyUpdates = updatedData.monthlyUpdates;
                selectedObjective.lastUpdated = new Date().toISOString().split('T')[0]; // Update lastUpdated to today

                // Update sessionStorage with the updated objective
                updateObjectiveInSession(selectedObjective);
                console.log('Updated objective in sessionStorage');
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.success) {
                ...
                deleteObjectiveFromSession(selectedObjective.id);
                console.log('Deleted objective from sessionStorage');
